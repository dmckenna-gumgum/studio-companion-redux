...
const {getLayerContainer, findValidGroups, toggleHistory, checkForExistingLayers, getLayerIndex, duplicateAndMoveToBottom, placeAtCorrectDepth } = require('../helpers/helpers.js');
...
async function propagateMissingLayersAction(validTypes, missingOnly) {
    console.log("(Action Script) propagateMissingLayersAction started.");
    ...
    try {
        const result = await core.executeAsModal(async (modalExecutionContext) => {
            console.log("(Modal Action) Executing propagateMissingLayersAction within modal.");
                    ...
                    for (const sourceLayer of originalSelection) {
                        try {
                            ...
                            try {
                                ...
                                await placeAtCorrectDepth(duplicatedLayer, targetContainer, sourceIndex, distanceFromBottom);
                            } catch (reorderError) {
        ...
        console.log("(Action Script) propagateMissingAction finished. Result:", result);
        return result;

    } catch (error) {
        console.error("(Action Script) Error in propagateMissingAction:", error);
...
module.exports = propagateMissingLayersAction;
